...
const ensureCanChangeAdminState = async (userId, companyId, nextRole, nextActive) => {
    ...
    if (!currentlyAdmin)
        return;
    if (willRemainAdmin && willBeActive)
        return;
...
export const updateUserRole = async ({ currentUserId, currentCompanyId, userId, role }) => {
...
export const updateUserStatus = async ({ currentUserId, currentCompanyId, userId, isActive }) => {
    assertObjectId(userId, 'user id');
