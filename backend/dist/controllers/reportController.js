...
export const taxSummaryHandler = asyncHandler(async (req, res) => {
    const data = await getTaxSummary(readPeriod(req));
    res.json({ success: true, data });
});
export const alertsHandler = asyncHandler(async (_req, res) => {
    const data = await listAlerts();
    res.json({ success: true, data });
});
export const getCompanySettingsHandler = asyncHandler(async (_req, res) => {
...
export const vatReturnHandler = asyncHandler(async (req, res) => {
    ...
    if (format === 'pdf') {
        ...
        for (const expense of vatReturn.expenses) {
            pdf.text(`${new Date(expense.date).toISOString().slice(0, 10)} | ${expense.description} | Amount: ${expense.amount.toFixed(2)} | Input VAT: ${expense.inputVAT.toFixed(2)}`);
        }
        pdf.moveDown();
...
export const citReturnHandler = asyncHandler(async (req, res) => {
