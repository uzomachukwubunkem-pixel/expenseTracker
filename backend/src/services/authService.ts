...
const normalizeCompanyId = (value: string): string => value.trim().toLowerCase()
const normalizeEmail = (value: string): string => value.trim().toLowerCase()
const generateCompanyId = (): string => `cmp-${randomId()}`
const generatePendingCompanyId = (): string => `pending-${randomId()}`

const generateVerificationCode = (): string =>
  Math.floor(100000 + Math.random() * 900000).toString()

const issueEmailVerificationCode = async (user: { _id: unknown; name: string; email: string }) => {
...
const issuePasswordResetToken = async (user: { _id: unknown; name: string; email: string }) => {
...
export const register = async (payload: unknown) => {
  ...
  const resolvedRole: 'admin' | 'staff' = parsed.role ?? 'staff'
  ...
  if (resolvedRole === 'staff') {
    ...
    if (requestedCompanyId) {
      throw new AppError(
        'Admin accounts receive a company ID after company setup. Leave company ID blank during account registration.',
        400,
      )
    }
...
export const sendVerificationCode = async (payload: unknown) => {
...
export const verifyEmailCode = async (payload: unknown) => {
...
export const requestPasswordReset = async (payload: unknown) => {
...
export const resetPassword = async (payload: unknown) => {
  ...
  if (!user) throw new AppError('Password reset request not found', 404)
...
export const rotateRefreshToken = async (refreshToken: string) => {
  ...
  if (!activeToken) throw new AppError('Invalid refresh token', 401)
