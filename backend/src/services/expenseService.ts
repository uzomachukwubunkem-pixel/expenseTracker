...
export const listExpenses = async (userId: string, query: unknown) => {
...
export const createExpense = async (userId: string, payload: unknown) => {
...
export const updateExpense = async (expenseId: string, userId: string, payload: unknown) => {
  const parsed = expenseSchema.partial().parse(payload)

  const expense = await ExpenseModel.findOneAndUpdate(
    { _id: expenseId, user: userId, deletedAt: null },
    { $set: parsed },
    {
      new: true,
      runValidators: true,
...
export const softDeleteExpense = async (expenseId: string, userId: string) => {
  const expense = await ExpenseModel.findOneAndUpdate(
    { _id: expenseId, user: userId, deletedAt: null },
