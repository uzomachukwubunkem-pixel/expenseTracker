...
import {
  ...
  estimateCITForProfit,
  isCITExemptForTurnover,
  isVATExemptForTurnover,
} from './taxService'
...
export const upsertCompanySettings = async (payload: CompanySettingsInput) => {
  ...
  if (!legalName) throw new AppError('Legal name is required', 400)
  if (!taxId) throw new AppError('Tax ID is required', 400)
