...
const makeInvoiceNumber = (sequence: number): string => `INV-${String(sequence).padStart(6, '0')}`

export const createInvoice = async (userId: string, payload: unknown) => {
  const parsed = invoiceSchema.parse(payload)

  const company = await CompanySettingsModel.findOne()
  if (!company) throw new AppError('Company settings not initialized', 400)
